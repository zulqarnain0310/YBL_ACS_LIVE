	...
	;WITH CTE_OTS AS(
				SELECT APPLICANT_UCIC FROM PRO.COBORROWERDETAILS A 
						WHERE APPLICANT_UCIC NOT IN (SELECT REFCUSTOMERID FROM #CUST_SELF_NPA B) 
								AND FLGDEG='Y' AND DEGDATE IS NOT NULL AND AGREEMENTNO  IN (SELECT ACCOUNTID FROM OTS_DATA WHERE OTS_Settlement_Flag ='Y')--ADDED BY ZAIN AND BALA ON COBORROWER OBSERVATION
