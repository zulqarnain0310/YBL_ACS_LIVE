...
Create PROC [dbo].[Rpt-SysReportVersion_No_Timekey_28032024_bkp]      

 @ReportName AS VARCHAR(MAX)
  as


--DECLARE @ReportName AS VARCHAR(MAX)='Rpt-20042-MOC Report'

 DECLARE @TimeKey AS INT=(SELECT EffectiveToTimeKey FROM SysReportDirectory WHERE ReportRdlName=RIGHT(@reportname,charindex('-',reverse(@reportname))-1) )
 --SELECT @TimeKey,@ReportName
