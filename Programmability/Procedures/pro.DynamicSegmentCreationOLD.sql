...
CREATE PROCEDURE [pro].[DynamicSegmentCreationOLD]   
AS  
BEGIN  
...
SELECT Rec.CustomerID, ROW_NUMBER() OVER (ORDER BY Rec.CustomerID) SrNo  
...
INNER JOIN (SELECT customerID   
            ...
            GROUP BY customerID) TXN ON Rec.customerID=TXN.CustomerID  
INNER JOIN (SELECT CustomerID   
            FROM dbo.AdvAcCCDemandDetail  
            WHERE ISNULL(BalanceDemand,0)>0  
            GROUP BY CustomerID) Dmd ON Dmd.CustomerID=Rec.CustomerID                            
WHERE ISNULL(BalRecovery,0) > 0  
GROUP BY Rec.CustomerID  
  
Declare @NoofSegments TinyInt = 10  

TRUNCATE TABLE DemandRunSegmentMark  
  
INSERT INTO DemandRunSegmentMark (CustomerID, SrNo, SegmentNo)  
SELECT AeD.CustomerID ,aed.SrNo, NTILE(@NoofSegments) OVER (ORDER BY aed.SrNo) SegmentNo  
  ...
  AND RecDate <= @ProcessDate  
      ...
      BEGIN  
         ...
         /*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*/  
            ...
            SELECT ROW_NUMBER() OVER (ORDER BY RecDate) SrNo, *  
            INTO #RecoveryAmtList  
            FROM AdvAcCCRecoveryDetail_Seg'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+'  
            WHERE CUSTOMERID = @CUSTOMERID  
            AND ISNULL(BalRecovery,0) > 0  
            AND RecDate <= @ProcessDate  
            AND RecDate >= (SELECT MIN(DemandDate)  
         ...
         /*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*/  
                     ...
                     -- Update Recovery date, Adjustment data and Recovery amount  
                        ...
                        BEGIN  
                           ...
                           INSERT INTO AdvAcCCDemandDetail_Seg'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+'(customerid,DemandType,DemandDate,DemandAmt,RecDate,RecAdjDate,RecAmount,BalanceDemand,DmdSchNumber,AcType)  
