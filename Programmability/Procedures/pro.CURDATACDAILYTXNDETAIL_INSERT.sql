...
CREATE PROCEDURE [pro].[CURDATACDAILYTXNDETAIL_INSERT]
AS
BEGIN
   BEGIN TRY
...
DECLARE @TableName varchar(50)='.dbo.AcDailyTxnDetail_' 
...
Declare @InsertSQL Varchar(Max),@DeleteSQL Varchar(Max)
 ...
 END
	...
	Update YBL_ACS_MIS.DBO.ODS_FCR_CH_NOBOOK_CURR set rat_conv_TCLCY=102	FROM YBL_ACS_MIS.DBO.ODS_FCR_CH_NOBOOK_CURR WHERE ISNULL(rat_conv_TCLCY,0)>102
...
INSERT INTO PRO.PROCESSMONITOR(USERID,DESCRIPTION,MODE,STARTTIME,ENDTIME,TIMEKEY,SETID)
	SELECT ORIGINAL_LOGIN(),'ProductCode in (605,869,891,703,704,705,209) Not Consider Credit on AcDailyTxnDetail','RUNNING',GETDATE(),NULL,@TIMEKEY,@SETID
...
and TxnType ='CREDIT' and B.ProductCode in ('605','869','891','703','704','705','209')


UPDATE PRO.PROCESSMONITOR SET ENDTIME=GETDATE() ,MODE='COMPLETE' WHERE IDENTITYKEY = (SELECT IDENT_CURRENT('PRO.PROCESSMONITOR'))
 AND  TIMEKEY=@TIMEKEY AND DESCRIPTION='ProductCode in (605,869,891,703,704,705,209) Not Consider Credit on AcDailyTxnDetail'
-------------------
...
INSERT INTO PRO.PROCESSMONITOR(USERID,DESCRIPTION,MODE,STARTTIME,ENDTIME,TIMEKEY,SETID)
	SELECT ORIGINAL_LOGIN(),'ProductCode in (605,869,891,703,704,705,209) Not Consider Int on AcDailyTxnDetail','RUNNING',GETDATE(),NULL,@TIMEKEY,@SETID
...
A.CustomerAcID=b.AccountID
where  EXTDATE= @PROCESSDATE
and TxnSubType ='INTEREST' and B.ProductCode in ('605','869','891','703','704','705','209')

UPDATE PRO.PROCESSMONITOR SET ENDTIME=GETDATE() ,MODE='COMPLETE' WHERE IDENTITYKEY = (SELECT IDENT_CURRENT('PRO.PROCESSMONITOR'))
 AND  TIMEKEY=@TIMEKEY AND DESCRIPTION='ProductCode in (605,869,891,703,704,705,209) Not Consider Int on AcDailyTxnDetail'
