...
--sp_rename 'SecurityStatusMaster_MasterSearchList','SecurityStatusMaster_MasterSearchList_04052022'
---
--USE YES_MISDB
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
create PROC [dbo].[SecurityStatusMaster_MasterSearchList]
--Declare
...
/*  IT IS Used FOR GRID Search which are not Pending for Authorization And also used for Re-Edit    */
			...
			IF(@OperationFlag not in (16,17,20))
			 ...
			 IF OBJECT_ID('TempDB..#temp') IS NOT NULL
                 DROP TABLE  #temp;
                 SELECT			A.SecurityStatusAltKey,
							A.SecurityStatusDescription,
                 ...
                 (
                     SELECT 
							...
							--select *
                     FROM DimSecurityStatusMaster A
...
--,Change varchar(max)
                     ...
                     SELECT 	A.SecurityStatusAltKey,
							...
							,ISNULL(A.ApprovedBy,A.ModifiedBy) as ModAppBy
							,ISNULL(A.DateApproved,A.DateModified) as ModAppDate
                     FROM DimSecurityStatusMaster_Mod A
				 
									 WHERE A.EffectiveFromTimeKey <= @TimeKey  AND A.EffectiveToTimeKey >= @TimeKey
					     

                                  AND ISNULL(A.AuthorisationStatus, 'A') IN('NP', 'MP', 'DP', 'RM','1A')
                           AND A.SecurityStatusAltKey IN
                     (
                         SELECT MAX(SecurityStatusAltKey)
                         FROM DimSecurityStatusMaster_Mod
                         WHERE EffectiveFromTimeKey <= @TimeKey
                               AND EffectiveToTimeKey >= @TimeKey
                               AND ISNULL(AuthorisationStatus, 'A') IN('NP', 'MP', 'DP', 'RM','1A')
                         GROUP BY SecurityStatusAltKey
                     )
                 ) A 
                      
                 
                 GROUP BY A.SecurityStatusAltKey,
							A.SecurityStatusDescription,
                 ...
                 FROM
                 (
                     SELECT ROW_NUMBER() OVER(ORDER BY SecurityStatusAltKey) AS RowNumber, 
