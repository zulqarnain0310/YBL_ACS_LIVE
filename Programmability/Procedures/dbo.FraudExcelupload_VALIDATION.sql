...
CREATE Procedure [dbo].[FraudExcelupload_VALIDATION]
(
...
/*IF TABLE DOES EXISTS BUT STRUCTURE DOES NOT MATCH IN DB THEN DROP CREATE AS PER THE STG TABLE FORMAT*/
			IF (
				(SELECT COUNT(1) FROM SYS.COLUMNS A
					INNER JOIN SYS.TABLES B ON A.OBJECT_ID=B.OBJECT_ID
					WHERE B.NAME='FRAUDDATAUPLOAD')
					<>
				(SELECT COUNT(1) FROM SYS.COLUMNS A
					INNER JOIN SYS.TABLES B ON A.OBJECT_ID=B.OBJECT_ID
					WHERE B.NAME='FRAUDACCOUNTUPLOAD_STG')
			)
			BEGIN
				DROP TABLE DBO.FRAUDDATAUPLOAD
				SELECT * INTO DBO.FRAUDDATAUPLOAD FROM DATAUPLOAD.FRAUDACCOUNTUPLOAD_STG WHERE 1=2
			END
/*IF TABLE DOES EXISTS BUT STRUCTURE DOES NOT MATCH IN DB THEN DROP CREATE AS PER THE STG TABLE FORMAT END*/


/*ADDED NEW BY ZAIN ON 20241115*/

			IF (SELECT COUNT(1) FROM dbo.FRAUDDATAUPLOAD)>1
			BEGIN
				TRUNCATE TABLE dbo.FRAUDDATAUPLOAD
			END

/*LOADING DATA FROM STG TABLE TO UPLOAD TABLE*/
			INSERT INTO DBO.FRAUDDATAUPLOAD
			SELECT * FROM DATAUPLOAD.FRAUDACCOUNTUPLOAD_STG
/*LOADING DATA FROM STG TABLE TO UPLOAD TABLE*/

/*ADDED NEW BY ZAIN ON 20241115 END*/
...
/*ADDED NEW BY ZAIN ON 20241115*/
		...
		TRUNCATE TABLE DATAUPLOAD.FRAUDACCOUNTUPLOAD_STG
