...
CREATE PROCEDURE [pro].[Update_AssetClass_20250415]
@TIMEKEY INT
AS
BEGIN
    SET NOCOUNT ON
...
UPDATE A SET A.ASSET_NORM='ALWYS_NPA'
			...
			,FlgDeg=(CASE WHEN ISNULL(A.FlgDeg,'N')='N' THEN 'Y' ELSE A.FlgDeg END)
			,A.FINALNPADT=(CASE WHEN FINALNPADT IS NULL THEN @PROCESSDATE ELSE FINALNPADT END)
			,A.DPD_DCCO=(DATEDIFF(DAY,B.Final_DCCO,@PROCESSDATE))
			,A.FIN_DCCO_DATE=B.Final_DCCO
			,A.Utilisation=B.Utilisation
	FROM PRO.ACCOUNTCAL A INNER JOIN YBL_ACS..DCCO_MAIN B 
		ON A.RefCustomerID=B.Cust_ID AND (B.EFFECTIVEFROMTIMEKEY<=27484 AND B.EFFECTIVETOTIMEKEY>=27484)
	INNER JOIN DimSourceDB D ON A.SourceAlt_Key=D.SourceAlt_Key
			AND (D.EFFECTIVEFROMTIMEKEY<=27484 AND D.EFFECTIVETOTIMEKEY>=27484)
...
AND B.Utilisation>0
 ...
 UPDATE A SET 	A.ASSET_NORM='ALWYS_NPA'
            	,A.DEGREASON=' DEGRADE BY DCCO FOR UCIC/FCR_CUSTOMERID ' + D.SourceName+' '+A.RefCustomerID
				,FlgDeg=(CASE WHEN ISNULL(FlgDeg,'N')='N' THEN 'Y' ELSE FlgDeg END)
				,DegDate=(CASE WHEN DegDate IS NULL THEN @PROCESSDATE ELSE DegDate END)
		,A.SYSASSETCLASSALT_KEY=CASE WHEN SYSASSETCLASSALT_KEY=1 THEN 2 ELSE A.SYSASSETCLASSALT_KEY END --2  If Already Npa Customer resturcture marking done than asset class change based on updated Npa date
     		,A.SYSNPA_DT=(CASE WHEN SYSNPA_DT IS NULL THEN @PROCESSDATE ELSE SYSNPA_DT END)
