...
CREATE PROCEDURE [pro].[InsertDataforAssetClassficationYes_OPT]
--@TIMEKEY INT 
WITH RECOMPILE
AS
BEGIN 
...
SELECT ORIGINAL_LOGIN(),'UPDATE SOURCEALT_KEY IN CUSTOMER CAL TABLE','RUNNING',GETDATE(),NULL,@TIMEKEY,@SETID
	...
	---METAGRID TO GOLD
	/* 19072023 - AMAR COMMENTED BY AMAR - FOR OPTIMISATION -- BELOW CODE IS COVERED IN ABOVE UPDATE 
...
-- AND C.EFFECTIVEFROMTIMEKEY<=@TIMEKEY AND C.EFFECTIVETOTIMEKEY>=@TIMEKEY
	...
	UPDATE A SET A.CUSTOMERNAME=CASE WHEN A.CUSTOMERNAME IS NULL THEN B.COUNTERPARTLABEL ELSE A.CUSTOMERNAME END, /* 19072023 - AMAR ADDED FOR OPTIMISATION*/
				 A.REFCUSTOMERID=CASE WHEN A.CUSTOMERNAME IS NULL THEN B.FCC_CUSTOMERID ELSE  A.REFCUSTOMERID END /* 19072023 - AMAR ADDED FOR OPTIMISATION*/
				 ,A.DATEOFDATA=B.DATE_OF_DATA /* 19072023 - AMAR ADDED FOR OPTIMISATION*/
		 FROM   PRO.CUSTOMERCAL A 
	INNER JOIN  YBL_ACS_MIS..ODS_MUREX_DPD_NPA B 
	ON A.SOURCESYSTEMCUSTOMERID= B.FCC_CUSTOMERID
	INNER JOIN DIMSOURCEDB C ON A.SOURCEALT_KEY=C.SOURCEALT_KEY
	WHERE C.SOURCENAME = 'MUREX' ---AND A.CUSTOMERNAME IS NULL   /* 19072023 - AMAR ADDED FOR OPTIMISATION*/
	 AND C.EFFECTIVEFROMTIMEKEY<=@TIMEKEY AND C.EFFECTIVETOTIMEKEY>=@TIMEKEY
	 --AND B.MATCH_FCR='N'
...
UPDATE A  SET A.CUSTOMERNAME=CASE WHEN A.CUSTOMERNAME IS NULL THEN B.CUSTOMERNAME ELSE A.CUSTOMERNAME END
             ,A.CUSTSEGMENTCODE=CASE WHEN A.CUSTOMERNAME IS NULL THEN B.CUSTOMERBUSINESSSEGMENT ELSE A.CUSTSEGMENTCODE END
			 ,A.REFCUSTOMERID=CASE WHEN A.CUSTOMERNAME IS NULL THEN B.FCR_CUSTOMERID ELSE A.REFCUSTOMERID END
			 ,A.BRANCHCODE=CASE WHEN A.CUSTOMERNAME IS NULL THEN B.BRANCHCODE ELSE A.BRANCHCODE END
			 ,A.DATEOFDATA=B.Data_Date /* 19072023 - AMAR ADDED FOR OPTIMISATION*/
FROM PRO.CUSTOMERCAL A INNER JOIN YBL_ACS_MIS..CustomerData_CA B 
ON A.SOURCESYSTEMCUSTOMERID=B.SOURCESYSTEMCUSTOMERID
INNER JOIN DIMSOURCEDB C ON A.SOURCEALT_KEY=C.SOURCEALT_KEY
	AND C.EFFECTIVEFROMTIMEKEY<=@TIMEKEY AND C.EFFECTIVETOTIMEKEY>=@TIMEKEY
WHERE C.SOURCENAME = 'CREDAVENUE_DA'----AND A.CUSTOMERNAME IS NULL  /* 19072023 - AMAR ADDED FOR OPTIMISATION*/
...
SELECT ORIGINAL_LOGIN(),'UPDATE SRCASSETCLASSALT_KEY|SYSASSETCLASSALT_KEY|SYSNPA_DT|SRCNPA_DT','RUNNING',GETDATE(),NULL,@TIMEKEY,@SETID
	...
	----As per Bank Mail Only Src Asset Class and Date Should be Updated from Soure System Triloki Khanna 30112022
	/* 19072023 -AMAR COMMENTED FOR OPTIMISATION */
	/*
		UPDATE B SET B.SRCASSETCLASSALT_KEY= (CASE WHEN A.ASSETCLASS IN(1,2) THEN 1 
												   WHEN A.ASSETCLASS IN(3,4) THEN 2
												   WHEN ISNULL(A.ASSETCLASS,'N') ='Y' THEN 2
...
/* 19072023 - AMAR -- COMMENTED FOR OPTIMISATION - AS DISCUSSED WITH TUSHAR IT IS NOT USING
INSERT INTO PRO.PROCESSMONITOR(USERID,DESCRIPTION,MODE,STARTTIME,ENDTIME,TIMEKEY,SETID)
...
AND (	B.ACCOUNTBLKCODE1 in('W','G','O','U','T')
	  OR B.ACCOUNTBLKCODE2 in('W','G','O','U','T') -- ADDED BY AMAR - 19072023 FOR OPTIMISATION
	)

/* 19072023 - COMMENTER BY AMAR - CONDITION ADDED IN ABOVE CODE
DELETE A
FROM PRO.ACCOUNTCAL A INNER JOIN YBL_ACS_MIS..ACCOUNTDATA B
ON A.CUSTOMERACID=B.ACCOUNTID
where b.SourceSystemName='VISIONPLUS'
...
*/
		UPDATE A SET  A.CURRENTLIMIT=ISNULL(B.CURRENTLIMIT,0.00)
				 ...
				 ,A.ACTSEGMENTCODE=B.ACCOUNTSEGMENTCODE
				 ,A.LINECODE=(CASE WHEN SOURCENAME in ('FCC','FCR')  THEN    B.FACILITYCODELINECODE  ELSE NULL END)
				 /*19072023 - amar for optimisation added these update from below update */
...
/* 19072023 - amar commented for optimisation -  and below columnes included in above update
...
/* 19072023 - amar commented for optimisation -  and below columnes included in above upd
UPDATE A SET A.CONTIEXCESSDT=CASE WHEN C.SOURCENAME ='FCR' THEN  B.CONTINUOUSEXCESSSINCEDATE ELSE NULL END 
            ,A.LASTCRDATE =B.LASTCREDITDATE
			,A.STOCKSTDT  =B.STOCKSTATEMENTDATE
			,A.INTTRATE=ISNULL(B.INTERESTRATE,0.00)
...
/* 19072023 - AMR - CIMMENTED BELOW CODE FOR OPTMISATION
...
*/
/* 19072023 - AMAR - ADDED BELOW CODE FOR OPTIMISATION */
UPDATE A SET A.OVERDUESINCEDT=DATEADD(DAY,-MAXDPD,@PROCESSINGDATE)
FROM PRO.ACCOUNTCAL A INNER JOIN YBL_ACS_MIS..ACCOUNTDATA B ON A.CUSTOMERACID=B.ACCOUNTID
INNER JOIN DIMSOURCEDB C ON C.SOURCEALT_KEY=A.SOURCEALT_KEY
AND (C.EFFECTIVEFROMTIMEKEY<=@TIMEKEY AND C.EFFECTIVETOTIMEKEY>=@TIMEKEY)
WHERE C.SOURCENAME IN('VISIONPLUS','FINNONE','GANASEVA') AND ISNULL(B.MAXDPD,0)>0
...
FROM PRO.ACCOUNTCAL A 
--INNER JOIN YBL_ACS_MIS..ACCOUNTDATA B ON
...
AND (D.EFFECTIVEFROMTIMEKEY<=@TIMEKEY AND D.EFFECTIVETOTIMEKEY>=@TIMEKEY)
and D.SourceName=c.SrcSysProductName---New Condition Added Same product codes in same source system
...
WHERE  ( 
		   LineCode like'%294OD%' 
		OR LineCode like'%LDAG%' 
		OR LineCode like'%FCYAG%' 
		OR LineCode like'%226TLA%' 
		OR LineCode like'%IBUOD%'  
		OR LineCode like'%ODAG%'
		)
...
'Cash Margin','Fixed Deposits','Goods Vehicle','Passenger Vehicle','Immovable Plant and Machinery','Windmill')
where Adv.EffectiveFromTimeKey < = @TimeKey AND Adv.EffectiveToTimeKey > = @TimeKey)Advsec
INNER JOIN AdvSecurityValueDetail Sec ON (SEC.EffectiveFromTimeKey < = @TimeKey AND SEC.EffectiveToTimeKey >= @TimeKey)
						AND Advsec.CollateralID=Sec.CollateralID
						AND Advsec.SecurityEntityID=Sec.SecurityEntityID
...
INNER JOIN DIMPRODUCT C ON C.PRODUCTCODE=B.PRODUCTCODE
 AND (C.EFFECTIVEFROMTIMEKEY<=@TIMEKEY AND C.EFFECTIVETOTIMEKEY >=@TIMEKEY)
  ...
  AND (D.EFFECTIVEFROMTIMEKEY<=@TIMEKEY AND D.EFFECTIVETOTIMEKEY>=@TIMEKEY)
 SELECT SourceAlt_Key,ProductCode, * FROM PRO.ACCOUNTCAL
...
/* 19072023 - amar - added for optimisation */
	...
	WHERE (ISNULL(CurrentValue,0)>=ISNULL(B.Balance,0) AND ISNULL(B.Balance,0)<>0 ) 
		OR (ISNULL(CurrentValue,0)>0 AND ISNULL(B.Balance,0)=0)
	
/* 19072023  - amar commented below code for optimisation -- above code is added for below updates
UPDATE B SET Asset_Norm='ALWYS_STD'
 FROM ##AdvSecurityDetailAccountLevel A
INNER JOIN PRO.AccountCal B
ON A.CUSTOMERACID=B.CUSTOMERACID
...
FROM PRO.ACCOUNTCAL A INNER JOIN 
		(
			SELECT  UCIF_ID ,SUM(DEMANDAMT) DEMANDAMT,MIN(DEMANDDATE) DEMANDDATE 
...
----Due to OutofOrder 10-03-2023------------------
	...
	UPDATE A SET A.OVERDUEAMT=B.DEMANDAMT
            ,A.INTNOTSERVICEDDT=B.DEMANDDATE
	FROM PRO.ACCOUNTCAL A  
	INNER JOIN  CTE_DMD B 	/* 19072023 - AMAR ADDED FOR OPTIMISATION*/
		  ON A.CUSTOMERACID=B.CUSTOMERACID 
			/* 19072023 - AMAR COMMENTED FOR OPTIMISATION*/
			/*
				(
...
WHERE  A.SourceAlt_Key=1 and A.ProductCode in('660','661','889','681','682','693','694','695','696','715','716','717','718',
			     ...
			     '797','798','799','220','237','740','778','235') AND A.EFFECTIVEFROMTIMEKEY>26724
...
select CustomerAcID into #last90DebitBalance2  from #last90DebitBalance where cnt>=90
...
and A.ProductCode not in('660','661','889','681','682','693','694','695','696','715','716','717','718',
			     '755','756','758','763','764','765','766','787','788','789','795','796',
			     '797','798','799','220','237','869','219','819','891','703','704','705','209','605','740','778','235') AND A.EFFECTIVEFROMTIMEKEY>26724
...
-- UPDATE PRO.PROCESSMONITOR SET ENDTIME=GETDATE() ,MODE='COMPLETE' WHERE IDENTITYKEY = (SELECT IDENT_CURRENT('PRO.PROCESSMONITOR')) AND  TIMEKEY=@TIMEKEY AND DESCRIPTION='UPDATE DATEOFDATA IN ACCOUNT CAL TABLE'
 ...
 update pro.AccountCal set Balance=0,BalanceInCrncy =0 where sourcealt_key in (5,6,9)

 ---Balance update for ecbf and eifs
 Update A set Balance = ISNULL(B.TOTALBALANCEOUTSTANDINGINR,0.00) * -1
from pro.AccountCal A  inner join YBL_ACS_MIS.dbo.AccountData  B on A.customeracid=b.AccountID
where A.sourcealt_key in (5,6) and b.sourcesystemname in ('FCR')
...
AND C.EFFECTIVEFROMTIMEKEY<=@TIMEKEY AND C.EFFECTIVETOTIMEKEY>=@TIMEKEY
 ...
 UPDATE PRO.ACCOUNTCAL SET REFCUSTOMERID=SOURCESYSTEMCUSTOMERID WHERE REFCUSTOMERID='' AND SOURCEALT_KEY =10
		...
		AND B.SOURCENAME='VISIONPLUS' 
		AND (	
				(A.ACCOUNTSTATUS='Z' and ISNULL(BALANCE,0) > 0)---TotalBalanceOutstandingINR Change to BALANCE ENPA System column
			 OR (A.AccountBlkCode2='F')
			)
...
----END  
----FROM DPD_MORATORIUM_ADJUSTED Â A											
...
/**********execute CoBorrower sp************Pranay 2023-06-28****************/
EXEC [PRO].[CoBorrowerDetails_Insert]
