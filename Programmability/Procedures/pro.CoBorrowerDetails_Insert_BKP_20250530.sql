...
CREATE PROC [pro].[CoBorrowerDetails_Insert_BKP_20250530] 
...
/*ADDED FOR COBORROWER SOURCE INTEGRATION ON LOCAL NEW 20250401 BY ZAIN */
UNION ALL 
...
FROM YBL_ACS_MIS.dbo.COBORROWER_DATA_ELCM AA
UNION ALL
SELECT AB.*, 'C' AS StatusFlag,CAST(NULL AS VARCHAR(50)) AS SourceSystemCustomerID ,CAST(NULL AS VARCHAR(50)) AS UCIF_ID,CAST(NULL AS VARCHAR(50)) AS PANNO 
FROM YBL_ACS_MIS.dbo.COBORROWER_DATA_FINLAS AB
UNION ALL
SELECT AC.*, 'C' AS StatusFlag,CAST(NULL AS VARCHAR(50)) AS SourceSystemCustomerID ,CAST(NULL AS VARCHAR(50)) AS UCIF_ID,CAST(NULL AS VARCHAR(50)) AS PANNO 
FROM YBL_ACS_MIS.dbo.COBORROWER_DATA_NONFINLAS AC
)D
/*ADDED FOR COBORROWER SOURCE INTEGRATION ON LOCAL NEW 20250401 BY ZAIN END*/
...
IF OBJECT_ID('TEMPDB..#PrimaryUPGData') IS NOT NULL
  ...
  and b.UPGDate is null
	...
	IF @TIMEKEY =26851--26886 /* update FlgDeg ='Y' NAD deGdaTE AS fINALnPADATE FOR THE NPA ACCOUNT WHEN CO-BORROWER IMPLEMENTATION - TO BE EXECUTE ONLY ONCE */
		...
		--AND A.EffectiveFromTimeKey=@TIMEKEY AND A.EffectiveToTimeKey=@TIMEKEY /*COMMENTED BY ZAIN ON 20250402 IN LOCAL AS PER OBSERVATION RAISED ON UAT BY PRAVIN */
