...
=====================================================*/
CREATE PROCEDURE [pro].[DynamicSegmentCreation_FITL]   
AS  
BEGIN  
...
IF OBJECT_ID('TempDb..#ElegibleAccount_FITL') IS NOT NULL  
DROP TABLE #ElegibleAccount_FITL  
  
SELECT Rec.UCIF_ID, ROW_NUMBER() OVER (ORDER BY Rec.UCIF_ID) SrNo  
INTO #ElegibleAccount_FITL  
FROM AdvAcCCRecoveryDetail_FITL Rec  
...
INNER JOIN (SELECT UCIF_ID   
            FROM dbo.AdvAcCCDemandDetail_FITL  
...
TRUNCATE TABLE DemandRunSegmentMark_FITL  
  
INSERT INTO DemandRunSegmentMark_FITL (UCIF_ID, SrNo, SegmentNo)  
SELECT AeD.UCIF_ID ,aed.SrNo, NTILE(@NoofSegments) OVER (ORDER BY aed.SrNo) SegmentNo  
FROM #ElegibleAccount_FITL AeD  
  
SELECT SegmentNo, COUNT(1)  
FROM DemandRunSegmentMark_FITL  
GROUP BY SegmentNo  
ORDER BY SegmentNo  
...
set @Query = null   
select @Query ='  
IF OBJECT_ID(''dbo.AdvAcCCDemandDetail_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+''')  IS NOT NULL  
DROP TABLE dbo.AdvAcCCDemandDetail_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+'  
  
CREATE TABLE dbo.AdvAcCCDemandDetail_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+' (  
...
set @Query = null   
select @Query ='   
IF OBJECT_ID(''dbo.AdvAcCCRecoveryDetail_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+''')  IS NOT NULL  
DROP TABLE dbo.AdvAcCCRecoveryDetail_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+'  
  
CREATE TABLE dbo.AdvAcCCRecoveryDetail_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+'(  
 [EntityKey] [int] IDENTITY(1,1) NOT NULL,  
...
set @Query = null   
select @Query =   
'INSERT INTO AdvAcCCDemandDetail_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+'   
    ...
    SELECT  DMD.[CustomerID],[DemandType],[DemandDate],[DemandAmt],[RecDate],[RecAdjDate],[RecAmount]  
           ,[BalanceDemand],[DmdSchNumber],[AcType],DMD.[UCIF_ID], DMD.MnemonicCode 
FROM  AdvAcCCDemandDetail_FITL DMD  
INNER JOIN DEMANDRUNSEGMENTMARK_FITL B ON DMD.UCIF_ID = B.UCIF_ID  
...
set @Query = null   
select @Query =   
'INSERT INTO AdvAcCCRecoveryDetail_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+'   
           ([CustomerID],[RecAmt],[RecDate]  
           ,[BalRecovery],[UCIF_ID])  
 SELECT      [DMD].[CustomerID],[RecAmt],[RecDate]  
            ,[BalRecovery]  ,DMD.[UCIF_ID]  
FROM AdvAcCCRecoveryDetail_FITL DMD  
INNER JOIN DemandRunSegmentMark_FITL B ON DMD.UCIF_ID = B.UCIF_ID  
WHERE B.SegmentNo = '+CONVERT(VARCHAR(50),@Count)                       
...
select @Query =   
'  
CREATE CLUSTERED INDEX AdvAcCCDemandDetail_ctrl_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)  
+'  ON dbo.AdvAcCCDemandDetail_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+'    
(  
 [EntityKey] ASC  
...
CREATE NONCLUSTERED INDEX AdvAcCCDemandDetail_001_IX_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+'   
        ON dbo.AdvAcCCDemandDetail_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+ '  
(  
 [CustomerID] ASC,  
 [DemandDate] ASC,  
...
CREATE NONCLUSTERED INDEX AdvAcCCDemandDetail_002_IX_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+'    
    ON dbo.AdvAcCCDemandDetail_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+'    
(  
 [DemandDate] ASC  
)WITH (PAD_INDEX  = OFF, STATISTICS_NORECOMPUTE  = OFF, SORT_IN_TEMPDB = ON, IGNORE_DUP_KEY = OFF, DROP_EXISTING = OFF, ONLINE = ON, ALLOW_ROW_LOCKS  = ON, ALLOW_PAGE_LOCKS  = ON, FILLFACTOR = 80)   
...
CREATE NONCLUSTERED INDEX AdvAcCCDemandDetail_003_IX_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+'     
            ON dbo.AdvAcCCDemandDetail_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+'     
(  
 [CustomerID] ASC,  
 [DemandType] ASC  ,
...
CREATE CLUSTERED INDEX AdvAcCCRecoveryDetail_ctrl_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+'     
 ON dbo.AdvAcCCRecoveryDetail_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+'      
(  
 [EntityKey] ASC  
)WITH (PAD_INDEX  = OFF, STATISTICS_NORECOMPUTE  = OFF, SORT_IN_TEMPDB = ON, IGNORE_DUP_KEY = OFF, DROP_EXISTING = OFF, ONLINE = ON, ALLOW_ROW_LOCKS  = ON, ALLOW_PAGE_LOCKS  = ON, FILLFACTOR = 80)   
ON [PRIMARY]  
...
CREATE NONCLUSTERED INDEX AdvAcCCRecoveryDetail_001_IX_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+'      
        ON dbo.AdvAcCCRecoveryDetail_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+'       
(  
 [CustomerID] ASC,  
 [BalRecovery] ASC,  
...
'IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N''dbo.[DmdRecoSetOffLogicCC_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+']'') AND type in (N''P'', N''PC''))  
DROP PROCEDURE [dbo].[DmdRecoSetOffLogicCC_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+']'  
...
' /*===========================================
   ...
   DESCRIPTION : DEMAND RECOVERY SET OFF
    ================================================*/
CREATE PROCEDURE [dbo].[DmdRecoSetOffLogicCC_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+']    
                                            -- (   
              --@TimeKey INT  
            --,@UCIF_ID VARCHAR(30)  
           -- )  
...
SET NOCOUNT ON;  
  ...
  IF OBJECT_ID('+'''TEMPDB..#RecoveryAmtList'''+') IS NOT NULL
	...
	AS
	(
		SELECT UCIF_ID FROM AdvAcCCDemandDetail_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count) +'
			GROUP BY UCIF_ID 
	)
	  
  	SELECT ROW_NUMBER() OVER (ORDER BY A.UCIF_ID,RecDate) SrNo, A.*
        INTO #RecoveryAmtList  
     FROM AdvAcCCRecoveryDetail_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+' A   
  ...
  FROM #RecoveryAmtList    
      ...
      BEGIN  
         ...
         /*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*/  
          ...
          SELECT   @BalRecAmt = NULL   
           ...
           SELECT @Demand_Count = COUNT(1)  
                 , @Demand_Loop = 1  
           FROM AdvAcCCDemandDetail_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+' DmD 
         ...
         /*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*/  
                     ...
                     -- Update Recovery date, Adjustment data and Recovery amount  
                        ...
                        FROM AdvAcCCDemandDetail_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+' Dt_Demand  
           ...
           ,@Query_2=                    '-- If the excess demand has found then insert a new row for excess demand   
                        ...
                        BEGIN  
                           ...
                           INSERT INTO AdvAcCCDemandDetail_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+'(customerid,DemandType,DemandDate,DemandAmt,RecDate,RecAdjDate,RecAmount,BalanceDemand,DmdSchNumber,AcType,UCIF_ID,MnemonicCode)  
           ...
           -- Balance recovery update  
             ...
             FROM AdvAcCCRecoveryDetail_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+' Dt  
	...
	DELETE DMD
	FROM DemandRunSegmentMark_fitl A
	INNER JOIN dbo.AdvAcCCDemandDetail_FITL DMD ON A.UCIF_ID=DMD.UCIF_ID
	 DELETE REC
	FROM DemandRunSegmentMark_fitl A
	INNER JOIN dbo.AdvAcCCRecoveryDetail_FITL REC ON A.UCIF_ID=REC.UCIF_ID
...
--    EXEC [DmdRecoSetOffLogicCC_Seg_FITL'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+'] @TimeKey,@customerID
