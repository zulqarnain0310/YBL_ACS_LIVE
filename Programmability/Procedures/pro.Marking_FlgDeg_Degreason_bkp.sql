...
=============================================*/
CREATE PROCEDURE [pro].[Marking_FlgDeg_Degreason_bkp]
@TIMEKEY INT
AS
BEGIN
  SET NOCOUNT ON 
...
--AND C.SourceName='VisionPLUS' AND A.FinalAssetClassAlt_Key=1  AND  ISNULL(A.Balance,0)<=200
...
/* PHASE III ENPA WORK 24/11/2020 Added By Triloki Khanna*/

UPDATE A SET A.DEGREASON= ISNULL(A.DEGREASON,'')+', Technical NPA Due to Non-Renewal/Review'    --DEGRADE BY REVIEW DUE DATE
FROM PRO.AccountCal A INNER JOIN PRO.CustomerCal B ON A.RefCustomerID =B.RefCustomerID
