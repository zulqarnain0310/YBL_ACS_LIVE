...
=====================================================*/
CREATE PROCEDURE [pro].[DynamicSegmentCreation]   
AS  
BEGIN  
...
IF OBJECT_ID('TempElegibleAccount') IS NOT NULL
DROP TABLE TempElegibleAccount

select * into TempElegibleAccount from #ElegibleAccount
...
SET NOCOUNT ON;  
         ...
         /*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*/  
           ...
           ,@Query_2=                    '-- If the excess demand has found then insert a new row for excess demand   
                        ...
                        BEGIN  
                           ...
                           INSERT INTO AdvAcCCDemandDetail_Seg'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+'(customerid,DemandType,DemandDate,DemandAmt,RecDate,RecAdjDate,RecAmount,BalanceDemand,DmdSchNumber,AcType,UCIF_ID,MnemonicCode)  
  ...
  IF  OBJECT_ID(''BalanceRecoveryAmtList'') is not null
	...
	SELECT UCIF_ID,RECDATE,BalRecovery INTO BalanceRecoveryAmtList FROM AdvAcCCRecoveryDetail_Seg'+REPLICATE('0',(2-LEN(CONVERT(VARCHAR(50),@Count))))+CONVERT(VARCHAR(50),@Count)+' WHERE BalRecovery>0
	union all
