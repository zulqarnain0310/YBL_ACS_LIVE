...
CREATE PROCEDURE [dbo].[AbsoluteBackdatedMOC_InUp]
	 @MOCDATE Varchar(30)
	,@AccountID VARCHAR(30)
	,@Branchcode VARCHAR(30)
	,@CustomerID VARCHAR(30)
	,@TotalProvision DECIMAL(22,9)
	,@AdditionalProvision DECIMAL(22,9)
	,@FinalProvision DECIMAL(22,9)
...
--Condition Added to Remove iterations by shubham on 2024-04-27	
...
PRINT 'Records insertingfor NP/MP/DP'
	...
	DROP TABLE #AbsoluteBackdatedMOC
		...
		,@MOCREASON as MOCREASON --Added by Shubham on 2024-04-15 for addition of MOCREASON
		,NULL as isChanged 
...
Begin
    ...
    IF OBJECT_ID('Tempdb..#AuthRej') IS NOT NULL
       ...
       SELECT 
          ...
          ,[DateApproved]
       INTO #AuthRej
       from YBL_ACS.DataUpload.AbsoluteBackdatedMOC_mod
...
IF @OperationFlag=1 AND @AuthMode ='Y'
					...
					BEGIN
							...
							ELSE ---IF DATA IS AVAILABLE IN MAIN TABLE
								BEGIN
									...
									UPDATE YBL_ACS.DataUpload.AbsoluteBackdatedMOC
							...
							IF @OperationFlag=2
							BEGIN	
								UPDATE YBL_ACS.DataUpload.AbsoluteBackdatedMOC_Mod
		...
		BEGIN
				...
				BEGIN							
						...
						UPDATE YBL_ACS.DataUpload.AbsoluteBackdatedMOC SET
									ModifyBy =@Modifiedby 
									,DateModified=@DateModifie 
	...
	ELSE IF @OperationFlag=17 AND @AuthMode ='Y' 
		BEGIN
				...
				IF EXISTS(SELECT 1 FROM YBL_ACS.DataUpload.AbsoluteBackdatedMOC WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@Timekey))
		...
		BEGIN
				print 'S77' 
					...
					DECLARE @CurrRecordFromTimeKey smallint=0
						...
						SELECT @ExEntityKey= MAX(EntityKey) FROM YBL_ACS.DataUpload.AbsoluteBackdatedMOC_Mod 
						WHERE (EffectiveFromTimeKey<=@Timekey AND EffectiveToTimeKey >=@Timekey) 
							AND AuthorisationStatus IN('NP','MP','DP')	
							And CustomerACID = @AccountID
					...
					SELECT @ExEntityKey= MIN(EntityKey) FROM YBL_ACS.DataUpload.AbsoluteBackdatedMOC_Mod 
						WHERE (EffectiveFromTimeKey<=@Timekey AND EffectiveToTimeKey >=@Timekey)
						AND AuthorisationStatus IN('NP','MP','DP')	
						And CustomerACID = @AccountID
					...
					BEGIN	
						...
						IF EXISTS(SELECT 1 FROM YBL_ACS.DataUpload.AbsoluteBackdatedMOC_Mod WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey))
				...
				BEGIN
						...
						IF EXISTS(SELECT 1 FROM YBL_ACS.DataUpload.AbsoluteBackdatedMOC WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey) And CustomerACID = @AccountID --and datatype=@DataType
						)
							...
							BEGIN
								...
								IF EXISTS(SELECT 1 FROM YBL_ACS.DataUpload.AbsoluteBackdatedMOC WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey) 
									...
									BEGIN
									PRINT 'ITHE ALA'
									--delete from YBL_ACS.DataUpload.AbsoluteBackdatedMOC where EffectiveFromTimeKey=@TimeKey And CustomerACID = @AccountID --Commented by shubham as per observation against expected functionality

									
							INSERT INTO YBL_ACS.DataUpload.AbsoluteBackdatedMOC 
												(
                                                     [AccountEntityID]
													,[MOC_Date]
													,[UCIF_ID]
						...
						print 'a'
							UPDATE YBL_ACS.DataUpload.AbsoluteBackdatedMOC SET
										--EffectiveToTimeKey=@EffectiveFromTimeKey-1,   --Commented on 2024-04-12 by shubham Against Recordsd in main are expiring when authorising from Operation Screen
...
IF @ErrorHandle=0
	...
	print 'ShubhamJ123'
			INSERT INTO YBL_ACS.DataUpload.AbsoluteBackdatedMOC_Mod             
				(
													 [AccountEntityID]
													,[MOC_Date]
													,[UCIF_ID]
												...
												SELECT
					                                ...
					                                ,a.TotalProvision --Columns added to Remove iterations by shubham on 2024-04-27
													,@AccountID
													,a.BranchCode
													,[ExistingProvision]
													,[AdditionalProvision]
