...
Inner Join PRO.AccountCal_hist C ON C.CustomerAcID=B.RefSystemAcId 
...
----------ISNULL((SELECT SUM(ISNULL(CurntQtrRv,0))   FROM PRO.CUSTOMERCAL_HIST L    where L.UCIF_ID= ACH.UCIF_ID    
----------AND L.EFFECTIVEFROMTIMEKEY=@TIMEKEY AND L.EFFECTIVETOTIMEKEY=@TIMEKEY),0)   + 

ISNULL(ACH.SecurityValue,0)         AS [Security Value] ,  
           
CAST( ISNULL(ACH.PrincOutStd,0) /@Cost  AS DECIMAL(30,2))                      AS [Current Balance Outstanding],  
    
  
----CAST( ((ISNULL((SELECT SUM(ISNULL(CurntQtrRv,0)) FROM PRO.CUSTOMERCAL_HIST L where L.UCIF_ID= ACH.UCIF_ID   
----AND L.EFFECTIVEFROMTIMEKEY=@TIMEKEY AND L.EFFECTIVETOTIMEKEY=@TIMEKEY),0) + 

 ISNULL(ACH.SecurityValue,0)  /CAST(NULLIF( ISNULL(ACH.PrincOutStd,0) /@Cost,0)  AS DECIMAL(30,2)) *100                    AS [Security Erosion (%)],  
...
CONVERT(VARCHAR(20),ACH.FirstDtOfDisb,103)     as  [Date of Disbursement]

INTO  #MAINDATA
FROM  PRO.AccountCal_hist ACH    
LEFT JOIN PRO.CUSTOMERCAL_HIST CCH          ON ACH.SourceSystemCustomerID=CCH.SourceSystemCustomerID  
...
WHERE  ACH.FinalAssetClassAlt_Key>1  and ProductErosion='Y'  -----AND ACH.SourceAlt_Key in (1,2)   
