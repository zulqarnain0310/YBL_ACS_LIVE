...
CREATE PROCEDURE [pro].[DATATOCLIENTDEGRADEPNPA]
AS
BEGIN
  DECLARE @TIMEKEY INT = (SELECT TIMEKEY FROM PRO.EXTDATE_MISDB WHERE FLG = 'Y')
   -- DECLARE @PROCESSDATE DATE =(SELECT DATE FROM SYSDAYMATRIX WHERE TIMEKEY=@TIMEKEY)
  --IF (EOMONTH(@PROCESSDATE)=@PROCESSDATE  OR DATENAME(WEEKDAY, @PROCESSDATE)='FRIDAY')

BEGIN

DELETE  FROM  PRO.DataToClient_DegradePNPA WHERE TIMEKEY=@TIMEKEY 

INSERT INTO  PRO.DataToClient_DegradePNPA
...
WHERE  B.FlgPNPA='Y'  AND (ISNULL(A.Balance,0)>0 OR ISNULL(A.PrincOutStd,0)>0)
