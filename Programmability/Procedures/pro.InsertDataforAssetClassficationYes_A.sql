...
CREATE PROCEDURE [pro].[InsertDataforAssetClassficationYes_A]
--@TIMEKEY INT 
WITH RECOMPILE
AS
BEGIN 
...
AND  ISNULL(SOURCEALT_KEY,0) =0 --AND B.MATCH_FCR='N'
 ...
 --E-CBF TO ECBF
/*30012024 - AMAR - COMMENTED FOR ECBF- CUSTOMER WILL BE USED FROM FCR SOURCE*/
...
/*------- FOR SFIN AccountData_SmartFin 15102023---------------------------------*/
UPDATE  A SET A.SOURCEALT_KEY= C.SOURCEALT_KEY,A.DATEOFDATA=B.DATA_DATE
FROM PRO.ACCOUNTCAL A INNER JOIN YBL_ACS_MIS..AccountData_SmartFin B 
ON A.CUSTOMERACID=B.ACCOUNTID
INNER JOIN DIMSOURCEDB  C ON C.SOURCENAME=B.SOURCESYSTEMNAME
 AND C.EFFECTIVEFROMTIMEKEY<=@TIMEKEY AND C.EFFECTIVETOTIMEKEY>=@TIMEKEY
...
UPDATE A SET 
			...
			,A.ACCOUNTBLKCODE2       = (CASE WHEN SOURCENAME='VISIONPLUS'  THEN    B.ACCOUNTBLKCODE2	 ELSE NULL END)
			,A.PRODUCTCODE          = B.PRODUCTCODE
FROM PRO.ACCOUNTCAL A INNER JOIN YBL_ACS_MIS..AccountData_SmartFin B ON A.CUSTOMERACID=B.ACCOUNTID
INNER JOIN  DIMSOURCEDB C  ON C.SOURCEALT_KEY=A.SOURCEALT_KEY
AND (C.EFFECTIVEFROMTIMEKEY<=@TIMEKEY AND C.EFFECTIVETOTIMEKEY>=@TIMEKEY)
...
/*------- FOR SFIN AccountData_SmartFin 15102023---------------------------------*/
UPDATE A SET A.CURRENCYALT_KEY=ISNULL(C.CURRENCYALT_KEY,62) FROM  PRO.ACCOUNTCAL A INNER JOIN YBL_ACS_MIS..AccountData_SmartFin B 
ON A.CUSTOMERACID=B.ACCOUNTID
INNER JOIN DIMCURRENCY C ON C.CURRENCYCODE=B.CURRENCYCODE
AND C.EFFECTIVEFROMTIMEKEY<=@TIMEKEY AND C.EFFECTIVETOTIMEKEY>=@TIMEKEY
/*------- FOR SFIN AccountData_SmartFin 15102023---------------------------------*/
...
UPDATE A SET A.PRODUCTALT_KEY=C.PRODUCTALT_KEY
FROM PRO.ACCOUNTCAL A INNER JOIN YBL_ACS_MIS..AccountData_SmartFin B ON
A.CUSTOMERACID=B.ACCOUNTID
INNER JOIN DIMPRODUCT C ON C.PRODUCTCODE=B.PRODUCTCODE
 AND (C.EFFECTIVEFROMTIMEKEY<=@TIMEKEY AND C.EFFECTIVETOTIMEKEY >=@TIMEKEY)
  INNER JOIN DIMSOURCEDB D ON D.SOURCEALT_KEY=A.SOURCEALT_KEY
...
UPDATE A SET A.REFCUSTOMERID=B.FCR_CustomerID
            ,A.SOURCESYSTEMCUSTOMERID=B.FCR_CustomerID
			,A.CUSTOMERENTITYID=C.CUSTOMERENTITYID
			,A.UCIF_ID=C.UCIF_ID
			,A.UCIFENTITYID=C.UCIFENTITYID
FROM PRO.ACCOUNTCAL A 
/*30012024 AMAR JOIN CHANGED------INNER JOIN YBL_ACS_MIS.[DBO].[ODS_ECBF_BORROWERMST] B 
ON A.CUSTOMERACID=B.CUSTOMERACID--BORROWERID*/
INNER JOIN YBL_ACS_MIS.[dbo].[AccountData_ECBF] B ON A.CustomerAcID=B.ACCOUNTID
INNER JOIN PRO.CUSTOMERCAL  C ON C.SOURCESYSTEMCUSTOMERID=B.FCR_CustomerID
...
UPDATE B SET BANKASSETCLASS=A.ASSETCLASS
FROM YBL_ACS_MIS..AccountData_SmartFin A
INNER JOIN  PRO.ACCOUNTCAL B ON A.ACCOUNTID=B.CUSTOMERACID 
INNER JOIN DIMSOURCEDB C ON C.SOURCEALT_KEY=B.SOURCEALT_KEY
AND (C.EFFECTIVEFROMTIMEKEY<=@TIMEKEY AND C.EFFECTIVETOTIMEKEY>=@TIMEKEY)
/*------- FOR SFIN AccountData_SmartFin 15102023---------------------------------*/
...
UPDATE A SET A.FACILITYTYPE =(CASE WHEN B.SOURCENAME='MUREX' THEN 'INV'
								  ...
								  WHEN B.SOURCENAME IN('SFIN') THEN 'TL' -- FOR SFIN AccountData_SmartFin 15102023
								  ELSE NULL END) 
...
Update A set Balance = ISNULL(C.TOTALBALANCEOUTSTANDINGINR,0.00) * -1
from pro.AccountCal A  
inner join YBL_ACS_MIS.dbo.AccountData_SmartFin  B 
on A.customeracid=B.AccountID
inner join YBL_ACS_MIS.dbo.AccountData C 
on C.ContractRefNo =B.ContractRefNo
AND  A.customeracid=C.AccountID
...
/**********execute CoBorrower sp************17102023****************/
/*EXEC [PRO].[CoBorrowerDetails_Insert] */ ---Amar - 04122023 Chaged by Amar -Now this SP calling shifted to PRO].[Final_AssetClass_Npadate] SP just before Calling of  EXEC PRO.COBORROWER_DEG_UPG_MARKING @TIMEKEY,'D'
