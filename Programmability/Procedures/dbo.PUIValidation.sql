...
CREATE PROCEDURE [dbo].[PUIValidation]
...
UPDATE A
	SET  
        ERROR=CASE WHEN ISNULL(ERROR,'')='' THEN 'RevisedSCOD should be greater than OriginalSCOD. Kindly check and upload again'     
						ELSE ERROR+','+SPACE(1)+'RevisedSCOD should be greater than OriginalSCOD. Kindly check and upload again'     END
		  
	FROM ##PUIData A
 WHERE (Case 
        When ISDATE(RevisedCompletionDt)=1 AND ISDATE(OriginalEnvisagCompletionDt)=1 Then 
		Case When Convert(date,RevisedCompletionDt)<=Convert(date,OriginalEnvisagCompletionDt)     Then 1
       Else 0 END END)=1

/****************************************************************************************************************
					
											FOR CHECKING Actual DCCO

--****************************************************************************************************************/
UPDATE A
				SET ERROR = 
								CASE	WHEN ISNULL(ERROR,'')=''  AND ISNULL(A.ActualCompletionDt,'')<>'' AND ISNULL(B.correct,0)<>1 
											THEN 'Invalid ActualCompletionDt'

										WHEN ISNULL(ERROR,'')<>'' AND ISNULL(A.ActualCompletionDt,'')<>'' AND ISNULL(B.correct,0)<>1 
													THEN ISNULL(ERROR,'')+','+SPACE(1)+ 'Invalid ActualCompletionDt'

										--WHEN ISNULL(ERROR,'')=''  AND ISNULL(A.ActualCompletionDt,'')='' 
										--			THEN 'ActualCompletionDt cannot be empty'

										--WHEN ISNULL(ERROR,'')<>'' AND ISNULL(A.ActualCompletionDt,'')='' THEN 
										--			ISNULL(ERROR,'')+','+SPACE(1)+ 'ActualCompletionDt cannot be empty'
										--WHEN ISNULL(ERROR,'')<>'' AND (Convert(date,A.ActualCompletionDt,103)>convert(date,A.ActualCompletionDt,103)) THEN 
										--			ISNULL(ERROR,'')+','+SPACE(1)+ 'ActualCompletionDt cannot be empty'
			...
			--SELECT 1
				SELECT RowNum ,1 correct FROM ##PUIData
				WHERE ISDATE(ActualCompletionDt)=1
				AND (CASE	WHEN SUBSTRING(RTRIM(LTRIM(ActualCompletionDt)),3,1)='-' 
								AND (LEN(RTRIM(LTRIM(ActualCompletionDt)))=9 OR LEN(RTRIM(LTRIM(ActualCompletionDt)))=11 )
								AND ISNUMERIC(SUBSTRING(RTRIM(LTRIM(ActualCompletionDt)),4,3))=0 
								AND  SUBSTRING(RTRIM(LTRIM(ActualCompletionDt)),7,1)='-' 
							THEN 1

							WHEN SUBSTRING(RTRIM(LTRIM(ActualCompletionDt)),3,1)='/'
							AND (LEN(RTRIM(LTRIM(ActualCompletionDt)))=8 OR LEN(RTRIM(LTRIM(ActualCompletionDt)))=10 )
							 AND  SUBSTRING(RTRIM(LTRIM(ActualCompletionDt)),6,1)='/' THEN 1
					END)=1
			)B 
			ON A.RowNum = B.RowNum
			WHERE ISNULL(B.RowNum,'')='' 
...
****************************************************************************************************************/
	...
	UPDATE A
		SET ERROR = CASE	WHEN ISNULL(A.ERROR,'')='' 		THEN 'Project Delay should not be Empty'
							Else A.ERROR+','+SPACE(1)+'Project Delay Reason should not be Empty' END
							
		FROM ##PUIData A
		where A.StandardRestruct ='Yes'
		and ISNULL(a.ProjectDelReason,'')=''
