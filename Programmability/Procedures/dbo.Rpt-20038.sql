...
CREATE PROCEDURE [dbo].[Rpt-20038]
   @TimeKey AS INT,
-- @Source   VARCHAR(200),
...
--DECLARE 
--      @TimeKey AS INT =(select TimeKey from SysDayMatrix where date='15/jun/2024'),
...
--27062024 Added for FCNRTPSecurity.
IF OBJECT_ID('TEMPDB..##SecurityTP') IS NOT NULL     		
	DROP TABLE ##SecurityTP    		
SELECT SUM(ISNULL(L.CurrentValue,0)) as CurrentValue ,l.CustomerAcID      		
into ##SecurityTP     		
FROM curdat.AdvSecurityDetailAccountLevel L     		
where EffectiveFromTimeKey  >= @Timekey and EffectiveFROMTimeKey <=@Timekey     		
AND CREATEDBY='TP'		
group by l.CustomerAcID
--27062024 Added for FCNRTPSecurity.

--14062024 Added for 60 and 80 days old Credit debit.
declare @EndDt date=(select Date from SysDayMatrix where TimeKey=@TimeKey)
declare @StartDt date

if object_id('tempdb..#60DaysCreditDebit')is not null
	drop table #60DaysCreditDebit
create table #60DaysCreditDebit(
[AccountEntityID] [int] NOT NULL
,[CreditAmount] [decimal](38, 2) NULL
,[DebitAmount] [decimal](38, 2) NULL
)

if object_id('tempdb..#80DaysCreditDebit')is not null
	drop table #80DaysCreditDebit
create table #80DaysCreditDebit(
[AccountEntityID] [int] NOT NULL
,[CreditAmount] [decimal](38, 2) NULL
,[DebitAmount] [decimal](38, 2) NULL
)

set @StartDt=dateadd(DD,-60,@EndDt)
insert #60DaysCreditDebit
	exec dbo.CustomDaysCreditDebit @StartDt=@StartDt, @EndDt=@EndDt

set @StartDt=dateadd(DD,-80,@EndDt)
insert #80DaysCreditDebit
	exec dbo.CustomDaysCreditDebit @StartDt=@StartDt, @EndDt=@EndDt
--14062024 Added for 60 and 80 days old Credit debit.
...
isnull(ACCVAR.VariableCreditAmt,0)/@Cost AS [VariableDaysCreditAmt],
isnull(ACCVAR.VariableDebitAmt,0)/@Cost AS [VariableDaysdeditAmt],
...
case when ( a.LineCode like'%294ODAGFD%'  OR a.LineCode like'%ODAG-FCNR%' OR a.LineCode like'%IBUODAGFD%' 
 ...
 OR a.LineCode like'%226TLAGFD%' OR a.LineCode like'%FCYAG-DEP%' OR a.LineCode like'%LDAG-FCNR%'  OR a.LineCode like'%FD-EXCLE%'   
...
,A.DrawingPower--27062024 Added
,TP.CurrentValue as FCNRTPSecurity--27062024 Added
,ACCVAR.AccountMarking as [Marking]

--14062024 Added
,CR_DR80.CreditAmount as [80DaysCreditAmount]
,CR_DR80.DebitAmount as [80DaysDebitAmount]
,CR_DR60.CreditAmount as [60DaysCreditAmount]
,CR_DR60.DebitAmount as [60DaysDebitAmount]
...
--27062024 Added
left join ##SecurityTP TP  ON TP.CustomerAcID=A.CustomerAcID

--14062024 Added
left join #60DaysCreditDebit CR_DR60 on CR_DR60.AccountEntityID=A.AccountEntityID
left join #80DaysCreditDebit CR_DR80 on CR_DR80.AccountEntityID=A.AccountEntityID
