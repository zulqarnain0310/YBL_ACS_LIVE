USE [YBL_ACS]
GO
/****** Object:  StoredProcedure [pro].[Upgrade_Customer_Account]    Script Date: 7/14/2025 11:47:29 AM ******/
SET ANSI_NULLS ON
GO
...
=============================================*/
alter PROCEDURE [Pro].[Upgrade_Customer_Account]
@TIMEKEY INT
WITH RECOMPILE
AS
BEGIN
...
SELECT  UCIF_ID,COUNT(1) COUNT_STD INTO #ACCOUNT_ALWAYS_STD 
FROM PRO.AccountCal WHERE Asset_Norm='ALWYS_STD' AND UCIF_ID is not NULL 
GROUP BY UCIF_ID


IF OBJECT_ID('TEMPDB..#ACCOUNT_ALL') IS NOT NULL
				DROP TABLE #ACCOUNT_ALL
		
SELECT  A.UCIF_ID,COUNT(1) COUNT_ALL INTO #ACCOUNT_ALL 
FROM PRO.AccountCal A INNER JOIN #ACCOUNT_ALWAYS_STD B 
ON A.UCIF_ID=B.UCIF_ID GROUP BY A.UCIF_ID


;WITH CTE_STD AS(SELECT A.UCIF_ID FROM 	#ACCOUNT_ALWAYS_STD A
							INNER JOIN #ACCOUNT_ALL B ON A.UCIF_ID=B.UCIF_ID
						...
						ON 
						 A.UCIF_ID=B.UCIF_ID
						 where B.SysAssetClassAlt_Key>1 
