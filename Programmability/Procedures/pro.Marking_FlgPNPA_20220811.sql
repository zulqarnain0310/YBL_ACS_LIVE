...
/*=========================================
 ...
 DESCRIPTION : MARKING OF FlgPNPA AND DEG REASON 
  EXEC [PRO].[Marking_FlgPNPA] 25933
=============================================*/
CREATE PROCEDURE [pro].[Marking_FlgPNPA_20220811]
@TIMEKEY INT
with recompile
AS
BEGIN
...
--CHANGES DONE TO HANDLE PNPA FOR 61 DAYS TRILOKI 08092019---
...
DECLARE @PNPAPROCESSDATE DATE  =(SELECT DATE FROM SYSDAYMATRIX WHERE TIMEKEY=@TIMEKEY)--(SELECT EOMONTH(DATE) FROM SYSDAYMATRIX WHERE CAST(DATE AS DATE) =@PROCESSDATE)
DECLARE @PNPA_DAYS INT =DATEDIFF(DAY,@PROCESSDATE,@PNPAPROCESSDATE)


/*--------------------INTIAL LEVEL FlgPNPA SET N-------------------------------------------- */

--UPDATE A SET A.FlgPNPA='N',PNPA_DATE=NULL
...
/*-----------------UPDATE FlgPNPA FLAG AT CUSTOMER LEVEL-----------------------------------------*/
 
--UPDATE B SET B.FlgPNPA='N' FROM PRO.ACCOUNTCAL A INNER JOIN  PRO.CustomerCal B 
--ON A.RefCustomerID=B.RefCustomerID
--WHERE  B.FlgProcessing='N'


--UPDATE  A SET A.PNPA_Reason=NULL 
--FROM PRO.AccountCal A INNER JOIN PRO.CustomerCal B ON A.RefCustomerID =B.RefCustomerID
--WHERE (B.FlgProcessing='N')
...
--With  KCC POTENTIAL NPA ACCOUNT CONDITION CHANGES DONE 08092019 TRILOKI---

--UPDATE A SET A.FLGPNPA =(CASE   WHEN  ((A.DPD_INTSERVICE+@PNPA_DAYS)>=335)   THEN 'Y' 
--								WHEN  ((A.DPD_NOCREDIT+@PNPA_DAYS)>=335)       THEN 'Y'
--								WHEN  ((A.DPD_OVERDUE+@PNPA_DAYS) >=335)        THEN 'Y' 
--								WHEN  ((A.DPD_STOCKSTMT+@PNPA_DAYS)>=335)  THEN 'Y' 
--								WHEN  ((A.DPD_RENEWAL+@PNPA_DAYS)>=335)          THEN 'Y'
--								WHEN  ((A.DPD_Overdrawn+@PNPA_DAYS)>=335)          THEN 'Y'
--							ELSE 'N'  END)
--FROM PRO.ACCOUNTCAL A   INNER JOIN PRO.CustomerCal B ON A.RefCustomerID =B.RefCustomerID
...
--UPDATE A SET A.FLGPNPA =(CASE   WHEN  ((A.DPD_INTSERVICE+@PNPA_DAYS)>=61)   THEN 'Y' 
--								WHEN  ((A.DPD_NOCREDIT+@PNPA_DAYS)>=61)       THEN 'Y'
--								WHEN  ((A.DPD_OVERDUE+@PNPA_DAYS) >=61)        THEN 'Y' 
--								WHEN  ((A.DPD_STOCKSTMT+@PNPA_DAYS)>=61)  THEN 'Y' 
--								WHEN  ((A.DPD_RENEWAL+@PNPA_DAYS)>=61)          THEN 'Y'
--								WHEN  ((A.DPD_Overdrawn+@PNPA_DAYS)>=61)          THEN 'Y'
--							ELSE 'N'  END)
--FROM PRO.ACCOUNTCAL A   INNER JOIN PRO.CustomerCal B ON A.RefCustomerID =B.RefCustomerID
--INNER JOIN DimProduct C ON  A.ProductAlt_Key=C.ProductAlt_Key AND (C.EffectiveFromTimeKey<=@TimeKey AND C.EffectiveToTimeKey>=@TimeKey)
--WHERE  (a.FinalAssetClassAlt_Key IN(SELECT AssetClassAlt_Key FROM DimAssetClass WHERE AssetClassShortNameEnum IN('STD') and EffectiveFromTimeKey<=@TIMEKEY AND EffectiveToTimeKey>=@TIMEKEY))
...
UPDATE A SET A.PNPA_Reason= ISNULL(A.PNPA_Reason,'')+',DEGRADE BY INT NOT SERVICED' 
             ,A.PNPA_DATE=DATEADD(DAY,-(A.DPD_INTSERVICE+@PNPA_DAYS-365),@PNPAPROCESSDATE)
...
WHERE  (A.FlgPNPA='Y' AND ((A.DPD_INTSERVICE+@PNPA_DAYS)>=335)) 
AND (ISNULL(C.PRODUCTGROUP,'N')='KCC'
OR ((LineCode LIKE '%CROP_OD_F%' or LineCode LIKE '%CROP_DLOD%' or LineCode LIKE '%CROP_TL_F%'))
OR( (ACCOUNTSTATUS LIKE '%CROP LOAN (OTHER THAN PL%' OR ACCOUNTSTATUS LIKE '%CROP LOAN (PLANT N HORTI%' OR ACCOUNTSTATUS LIKE '%PRE AND POST-HARVEST ACT%'
 OR ACCOUNTSTATUS LIKE '%FARMERS AGAINST HYPOTHEC%' OR ACCOUNTSTATUS LIKE '%FARMERS AGAINST PLEDGE O%' OR ACCOUNTSTATUS LIKE '%PLANTATION/HORTICULTURE%'
...
UPDATE A SET A.PNPA_Reason= ISNULL(A.PNPA_Reason,'')+', DEGRADE BY CONTI EXCESS'
           ,A.PNPA_DATE=DATEADD(DAY,-(A.DPD_OVERDRAWN+@PNPA_DAYS-365),@PNPAPROCESSDATE)
FROM PRO.AccountCal A-- INNER JOIN PRO.CustomerCal B ON A.CustomerEntityID =B.CustomerEntityID -- Modification DONE 03/09/2019 TRILOKI
INNER JOIN DimProduct C ON  A.ProductAlt_Key=C.ProductAlt_Key AND (C.EffectiveFromTimeKey<=@TimeKey AND C.EffectiveToTimeKey>=@TimeKey)
WHERE  (A.FlgPNPA='Y' AND ((A.DPD_OVERDRAWN+@PNPA_DAYS)>=335)) 
AND (ISNULL(C.PRODUCTGROUP,'N')='KCC'
OR ((LineCode LIKE '%CROP_OD_F%' or LineCode LIKE '%CROP_DLOD%' or LineCode LIKE '%CROP_TL_F%'))
OR( (ACCOUNTSTATUS LIKE '%CROP LOAN (OTHER THAN PL%' OR ACCOUNTSTATUS LIKE '%CROP LOAN (PLANT N HORTI%' OR ACCOUNTSTATUS LIKE '%PRE AND POST-HARVEST ACT%'
 OR ACCOUNTSTATUS LIKE '%FARMERS AGAINST HYPOTHEC%' OR ACCOUNTSTATUS LIKE '%FARMERS AGAINST PLEDGE O%' OR ACCOUNTSTATUS LIKE '%PLANTATION/HORTICULTURE%'
...
UPDATE A SET A.PNPA_Reason= ISNULL(A.PNPA_Reason,'')+', DEGRADE BY OVERDUE'    
      ,A.PNPA_DATE=DATEADD(DAY,-(A.DPD_OVERDUE +@PNPA_DAYS-365 ),@PNPAPROCESSDATE)          
FROM PRO.AccountCal A --INNER JOIN PRO.CustomerCal B ON A.CustomerEntityID =B.CustomerEntityID -- Modification DONE 03/09/2019 TRILOKI
INNER JOIN DimProduct C ON  A.ProductAlt_Key=C.ProductAlt_Key AND (C.EffectiveFromTimeKey<=@TimeKey AND C.EffectiveToTimeKey>=@TimeKey)
WHERE  (A.FlgPNPA='Y' AND ((A.DPD_OVERDUE +@PNPA_DAYS)>=335))  
AND (ISNULL(C.PRODUCTGROUP,'N')='KCC'
OR ((LineCode LIKE '%CROP_OD_F%' or LineCode LIKE '%CROP_DLOD%' or LineCode LIKE '%CROP_TL_F%'))
OR( (ACCOUNTSTATUS LIKE '%CROP LOAN (OTHER THAN PL%' OR ACCOUNTSTATUS LIKE '%CROP LOAN (PLANT N HORTI%' OR ACCOUNTSTATUS LIKE '%PRE AND POST-HARVEST ACT%'
 OR ACCOUNTSTATUS LIKE '%FARMERS AGAINST HYPOTHEC%' OR ACCOUNTSTATUS LIKE '%FARMERS AGAINST PLEDGE O%' OR ACCOUNTSTATUS LIKE '%PLANTATION/HORTICULTURE%'
...
UPDATE A SET PNPA_Reason= ISNULL(A.PNPA_Reason,'')+', DEGRADE BY NO CREDIT'   
         ,A.PNPA_DATE=DATEADD(DAY,-(A.DPD_NOCREDIT+@PNPA_DAYS-365 ),@PNPAPROCESSDATE)
FROM PRO.AccountCal A --INNER JOIN PRO.CustomerCal B ON A.CustomerEntityID =B.CustomerEntityID -- Modification DONE 03/09/2019 TRILOKI
INNER JOIN DimProduct C ON  A.ProductAlt_Key=C.ProductAlt_Key AND (C.EffectiveFromTimeKey<=@TimeKey AND C.EffectiveToTimeKey>=@TimeKey)
WHERE  (A.FlgPNPA='Y' AND (A.DPD_NOCREDIT+@PNPA_DAYS)>=335)
AND (ISNULL(C.PRODUCTGROUP,'N')='KCC'
OR ((LineCode LIKE '%CROP_OD_F%' or LineCode LIKE '%CROP_DLOD%' or LineCode LIKE '%CROP_TL_F%'))
OR( (ACCOUNTSTATUS LIKE '%CROP LOAN (OTHER THAN PL%' OR ACCOUNTSTATUS LIKE '%CROP LOAN (PLANT N HORTI%' OR ACCOUNTSTATUS LIKE '%PRE AND POST-HARVEST ACT%'
 OR ACCOUNTSTATUS LIKE '%FARMERS AGAINST HYPOTHEC%' OR ACCOUNTSTATUS LIKE '%FARMERS AGAINST PLEDGE O%' OR ACCOUNTSTATUS LIKE '%PLANTATION/HORTICULTURE%'
...
UPDATE A SET PNPA_Reason= ISNULL(A.PNPA_Reason,'')+', DEGRADE BY STOCK STATEMENT' 
            ,A.PNPA_DATE=DATEADD(DAY,-(A.DPD_STOCKSTMT+@PNPA_DAYS-365 ),@PNPAPROCESSDATE)   
FROM PRO.AccountCal A ---INNER JOIN PRO.CustomerCal B ON A.CustomerEntityID =B.CustomerEntityID -- Modification DONE 03/09/2019 TRILOKI
INNER JOIN DimProduct C ON  A.ProductAlt_Key=C.ProductAlt_Key AND (C.EffectiveFromTimeKey<=@TimeKey AND C.EffectiveToTimeKey>=@TimeKey)
WHERE  (A.FlgPNPA='Y' AND (A.DPD_STOCKSTMT+@PNPA_DAYS)>=335)  
AND (ISNULL(C.PRODUCTGROUP,'N')='KCC'
OR ((LineCode LIKE '%CROP_OD_F%' or LineCode LIKE '%CROP_DLOD%' or LineCode LIKE '%CROP_TL_F%'))
OR( (ACCOUNTSTATUS LIKE '%CROP LOAN (OTHER THAN PL%' OR ACCOUNTSTATUS LIKE '%CROP LOAN (PLANT N HORTI%' OR ACCOUNTSTATUS LIKE '%PRE AND POST-HARVEST ACT%'
 OR ACCOUNTSTATUS LIKE '%FARMERS AGAINST HYPOTHEC%' OR ACCOUNTSTATUS LIKE '%FARMERS AGAINST PLEDGE O%' OR ACCOUNTSTATUS LIKE '%PLANTATION/HORTICULTURE%'
...
,A.PNPA_DATE=DATEADD(DAY,-(A.DPD_RENEWAL+@PNPA_DAYS-365 ),@PNPAPROCESSDATE)    
FROM PRO.AccountCal A-- INNER JOIN PRO.CustomerCal B ON A.CustomerEntityID =B.CustomerEntityID -- Modification DONE 03/09/2019 TRILOKI
INNER JOIN DimProduct C ON  A.ProductAlt_Key=C.ProductAlt_Key AND (C.EffectiveFromTimeKey<=@TimeKey AND C.EffectiveToTimeKey>=@TimeKey)
WHERE  (A.FlgPNPA='Y' AND (A.DPD_RENEWAL+@PNPA_DAYS)>=335)
AND (ISNULL(C.PRODUCTGROUP,'N')='KCC'
OR ((LineCode LIKE '%CROP_OD_F%' or LineCode LIKE '%CROP_DLOD%' or LineCode LIKE '%CROP_TL_F%'))
OR( (ACCOUNTSTATUS LIKE '%CROP LOAN (OTHER THAN PL%' OR ACCOUNTSTATUS LIKE '%CROP LOAN (PLANT N HORTI%' OR ACCOUNTSTATUS LIKE '%PRE AND POST-HARVEST ACT%'
 OR ACCOUNTSTATUS LIKE '%FARMERS AGAINST HYPOTHEC%' OR ACCOUNTSTATUS LIKE '%FARMERS AGAINST PLEDGE O%' OR ACCOUNTSTATUS LIKE '%PLANTATION/HORTICULTURE%'
...
UPDATE A SET A.PNPA_Reason= ISNULL(A.PNPA_Reason,'')+',DEGRADE BY INT NOT SERVICED' 
             ,A.PNPA_DATE=DATEADD(DAY,-(A.DPD_INTSERVICE+@PNPA_DAYS-91),@PNPAPROCESSDATE)
FROM PRO.AccountCal A ---INNER JOIN PRO.CustomerCal B ON A.CustomerEntityID =B.CustomerEntityID -- Modification DONE 03/09/2019 TRILOKI
INNER JOIN DimProduct C ON  A.ProductAlt_Key=C.ProductAlt_Key AND (C.EffectiveFromTimeKey<=@TimeKey AND C.EffectiveToTimeKey>=@TimeKey)
WHERE  (A.FlgPNPA='Y' AND ((A.DPD_INTSERVICE+@PNPA_DAYS)>=61)) 
AND ISNULL(C.PRODUCTGROUP,'N')<>'KCC'

UPDATE A SET A.PNPA_Reason= ISNULL(A.PNPA_Reason,'')+', DEGRADE BY CONTI EXCESS'
           ,A.PNPA_DATE=DATEADD(DAY,-(A.DPD_OVERDRAWN+@PNPA_DAYS-91),@PNPAPROCESSDATE)
FROM PRO.AccountCal A ---INNER JOIN PRO.CustomerCal B ON A.CustomerEntityID =B.CustomerEntityID -- Modification DONE 03/09/2019 TRILOKI
INNER JOIN DimProduct C ON  A.ProductAlt_Key=C.ProductAlt_Key AND (C.EffectiveFromTimeKey<=@TimeKey AND C.EffectiveToTimeKey>=@TimeKey)
WHERE (A.FlgPNPA='Y' AND ((A.DPD_OVERDRAWN+@PNPA_DAYS)>=61)) 
AND ISNULL(C.PRODUCTGROUP,'N')<>'KCC'

UPDATE A SET A.PNPA_Reason= ISNULL(A.PNPA_Reason,'')+', DEGRADE BY OVERDUE'    
      ,A.PNPA_DATE=DATEADD(DAY,-(A.DPD_OVERDUE +@PNPA_DAYS-91 ),@PNPAPROCESSDATE)          
FROM PRO.AccountCal A ---INNER JOIN PRO.CustomerCal B ON A.CustomerEntityID =B.CustomerEntityID -- Modification DONE 03/09/2019 TRILOKI
INNER JOIN DimProduct C ON  A.ProductAlt_Key=C.ProductAlt_Key AND (C.EffectiveFromTimeKey<=@TimeKey AND C.EffectiveToTimeKey>=@TimeKey)
WHERE  (A.FlgPNPA='Y' AND ((A.DPD_OVERDUE +@PNPA_DAYS)>=61))  
AND ISNULL(C.PRODUCTGROUP,'N')<>'KCC'
...
UPDATE A SET PNPA_Reason= ISNULL(A.PNPA_Reason,'')+', DEGRADE BY NO CREDIT'   
         ,A.PNPA_DATE=DATEADD(DAY,-(A.DPD_NOCREDIT+@PNPA_DAYS-91 ),@PNPAPROCESSDATE)
FROM PRO.AccountCal A --INNER JOIN PRO.CustomerCal B ON A.CustomerEntityID =B.CustomerEntityID -- Modification DONE 03/09/2019 TRILOKI
INNER JOIN DimProduct C ON  A.ProductAlt_Key=C.ProductAlt_Key AND (C.EffectiveFromTimeKey<=@TimeKey AND C.EffectiveToTimeKey>=@TimeKey)
WHERE   (A.FlgPNPA='Y' AND (A.DPD_NOCREDIT+@PNPA_DAYS)>=61)
AND ISNULL(C.PRODUCTGROUP,'N')<>'KCC'

UPDATE A SET PNPA_Reason= ISNULL(A.PNPA_Reason,'')+', DEGRADE BY STOCK STATEMENT' 
            ,A.PNPA_DATE=DATEADD(DAY,-(A.DPD_STOCKSTMT+@PNPA_DAYS-91 ),@PNPAPROCESSDATE)   
FROM PRO.AccountCal A ---INNER JOIN PRO.CustomerCal B ON A.CustomerEntityID =B.CustomerEntityID -- Modification DONE 03/09/2019 TRILOKI
INNER JOIN DimProduct C ON  A.ProductAlt_Key=C.ProductAlt_Key AND (C.EffectiveFromTimeKey<=@TimeKey AND C.EffectiveToTimeKey>=@TimeKey)
WHERE  (A.FlgPNPA='Y' AND (A.DPD_STOCKSTMT+@PNPA_DAYS)>=61)  
AND ISNULL(C.PRODUCTGROUP,'N')<>'KCC'

UPDATE A SET A.PNPA_Reason= ISNULL(A.PNPA_Reason,'')+', DEGRADE BY REVIEW DUE DATE'   
,A.PNPA_DATE=DATEADD(DAY,-(A.DPD_RENEWAL+@PNPA_DAYS-91 ),@PNPAPROCESSDATE)    
FROM PRO.AccountCal A--- INNER JOIN PRO.CustomerCal B ON A.CustomerEntityID =B.CustomerEntityID -- Modification DONE 03/09/2019 TRILOKI
INNER JOIN DimProduct C ON  A.ProductAlt_Key=C.ProductAlt_Key AND (C.EffectiveFromTimeKey<=@TimeKey AND C.EffectiveToTimeKey>=@TimeKey)
WHERE  (A.FlgPNPA='Y' AND (A.DPD_RENEWAL+@PNPA_DAYS)>=61) 
AND ISNULL(C.PRODUCTGROUP,'N')<>'KCC'
...
SELECT A.REFCUSTOMERID,MIN(A.PNPA_DATE) PNPA_DATE   INTO #TEMPTABLEPNPA   FROM PRO.ACCOUNTCAL A INNER JOIN  PRO.CUSTOMERCAL B 
ON A.REFCUSTOMERID=B.REFCUSTOMERID
WHERE B.FLGPNPA='Y' AND (B.FLGPROCESSING='N')
GROUP BY A.REFCUSTOMERID

UPDATE A SET A.PNPA_DT=B.PNPA_DATE
FROM PRO.CUSTOMERCAL A INNER JOIN #TEMPTABLEPNPA B ON A.REFCUSTOMERID=B.REFCUSTOMERID
...
WHERE ISNULL(A.Balance,0)>0  and b.FlgPNPA='Y'  AND (ISNULL(B.FlgProcessing,'N')='N')
 ...
 /*-------------------------START PUI PNPA PROCESS-------------------------*/

 
IF OBJECT_ID('TEMPDB..#TEMPTABLEPUI') IS NOT NULL
 DROP TABLE #TEMPTABLEPUI
...
,B.UcifEntityID
,ProjectCatgAlt_Key
,c.ParameterName AS ProjectCategory
,OriginalEnvisagCompletionDt
,RevisedCompletionDt
,ActualCompletionDt
,ProjectDelReason_AltKey
,d.ParameterName AS ProdectDelReson
,StandardRestruct_AltKey
,E.ParameterName AS StandardRestruct
INTO #TEMPTABLEPUI
FROM AdvAcProjectDetail A
INNER JOIN PRO.AccountCal B
ON A.RefAccountEntityId=B.AccountEntityID

LEFT OUTER JOIN  DIMPARAMETER C ON A.PROJECTCATGALT_KEY=C.PARAMETERALT_KEY
and c.DimParameterName='ProjectCategory'
And C.EffectiveFromTimeKey<=@TimeKey And C.EffectiveToTimeKey>=@TimeKey

LEFT OUTER JOIN  DIMPARAMETER D ON A.PROJECTDELREASON_ALTKEY=D.PARAMETERALT_KEY
and D.DimParameterName='ProdectDelReson'
And D.EffectiveFromTimeKey<=@TimeKey And D.EffectiveToTimeKey>=@TimeKey

LEFT OUTER JOIN  DIMPARAMETER E ON A.STANDARDRESTRUCT_ALTKEY=E.PARAMETERALT_KEY
and E.DimParameterName='DimYesNo'
And E.EffectiveFromTimeKey<=@TimeKey And E.EffectiveToTimeKey>=@TimeKey

WHERE (A.EffectiveFromTimeKey<=@TIMEKEY AND A.EffectiveToTimeKey>=@TIMEKEY)
AND B.AccountEntityID>0



----SC-1---
UPDATE B SET FlgPNPA='N'
FROM #TEMPTABLEPUI A
INNER JOIN PRO.AccountCal B
ON A.AccountEntityID=B.AccountEntityID
WHERE ProjectCategory='Infra' AND (isnull(B.FLGPNPA,'N')='N')
AND ISNULL(OriginalEnvisagCompletionDt,'1900-01-01')>@PROCESSDATE 
...
ON A.AccountEntityID=B.AccountEntityID
WHERE ProjectCategory='Infra' AND (isnull(B.FLGPNPA,'N')='N')
AND ActualCompletionDt IS NOT  NULL


----SC-3---
UPDATE B SET FlgPNPA='Y',PNPA_Reason='O_SCOD/ R_SCOD (whichever is latest) is a passed date, Actual DCCO Blank',PNPA_DATE=NULL
FROM #TEMPTABLEPUI A
INNER JOIN PRO.AccountCal B
ON A.AccountEntityID=B.AccountEntityID
WHERE ProjectCategory='Infra'
AND (isnull(B.FLGPNPA,'N')='N')
AND ISNULL(RevisedCompletionDt,OriginalEnvisagCompletionDt)<@PROCESSDATE 
AND ActualCompletionDt IS NULL 
AND ProdectDelReson='Beyond Control of Promoters'


----SC-4---
UPDATE B SET FlgPNPA='Y',PNPA_Reason='O_SCOD > 3 yrs, R_SCOD Blank, Actual DCCO Blank, Delay Reason - Other',PNPA_DATE=NULL
FROM #TEMPTABLEPUI A
INNER JOIN PRO.AccountCal B
ON A.AccountEntityID=B.AccountEntityID
WHERE ProjectCategory='Infra'
AND (isnull(B.FLGPNPA,'N')='N')
AND dateadd(year,3,OriginalEnvisagCompletionDt)<@PROCESSDATE 
AND ActualCompletionDt IS NULL 
AND RevisedCompletionDt IS NULL
AND ProdectDelReson='Other'
and OriginalEnvisagCompletionDt is not null


----SC-5---
UPDATE B SET FlgPNPA='Y',PNPA_Reason='R_SCOD > 2 yrs, Actual DCCO Bank, No Court case',PNPA_DATE=NULL
FROM #TEMPTABLEPUI A
INNER JOIN PRO.AccountCal B
ON A.AccountEntityID=B.AccountEntityID
WHERE ProjectCategory='Infra'
AND (isnull(B.FLGPNPA,'N')='N')
AND dateadd(year,2,RevisedCompletionDt)<@PROCESSDATE 
AND ActualCompletionDt IS NULL 
AND ProdectDelReson<>'Court Case'
and RevisedCompletionDt is not null

----SC-6---
UPDATE B SET FlgPNPA='Y',PNPA_Reason='R_SCOD > 3 yrs,  Actual DCCO Bank,  No Court case',PNPA_DATE=NULL
FROM #TEMPTABLEPUI A
INNER JOIN PRO.AccountCal B
ON A.AccountEntityID=B.AccountEntityID
WHERE ProjectCategory='Infra'
AND (isnull(B.FLGPNPA,'N')='N')
AND dateadd(year,3,RevisedCompletionDt)<@PROCESSDATE 
AND ActualCompletionDt IS NULL 
AND ProdectDelReson='Other'
and RevisedCompletionDt is not null
...
--------SC-7---
----UPDATE B SET FlgPNPA='Y',PNPA_Reason='R_SCOD <= 4 yrs, Actual DCCO Bank,  Court case',PNPA_DATE=NULL
----FROM #TEMPTABLEPUI A
----INNER JOIN PRO.AccountCal B
----ON A.AccountEntityID=B.AccountEntityID
----WHERE ProjectCategory='Infra'
----AND (isnull(B.FLGPNPA,'N')='N')
----AND dateadd(year,4,RevisedCompletionDt)<@PROCESSDATE 
----AND ActualCompletionDt IS NULL 
----AND ProdectDelReson='Court Case'
----and RevisedCompletionDt is not null
...
----SC-9---
UPDATE B SET FlgPNPA='Y',PNPA_Reason='O_SCOD > 1 yrs, R_SCOD Blank, Actual DCCO Bank, No Court case',PNPA_DATE=NULL
FROM #TEMPTABLEPUI A
INNER JOIN PRO.AccountCal B
ON A.AccountEntityID=B.AccountEntityID
WHERE ProjectCategory in('Non Infra','CRE')
AND (isnull(B.FLGPNPA,'N')='N')
AND dateadd(year,1,OriginalEnvisagCompletionDt)<@PROCESSDATE 
AND ProdectDelReson<>'Court Case'
and OriginalEnvisagCompletionDt is not null
and RevisedCompletionDt is  null
and ActualCompletionDt is  null
...
----SC-10---
UPDATE B SET FlgPNPA='Y',PNPA_Reason='O_SCOD > 2 yrs, R_SCOD Blank, Actual DCCO Bank,  No Court case',PNPA_DATE=NULL
FROM #TEMPTABLEPUI A
INNER JOIN PRO.AccountCal B
ON A.AccountEntityID=B.AccountEntityID
WHERE ProjectCategory in('Non Infra','CRE')
AND (isnull(B.FLGPNPA,'N')='N')
AND dateadd(year,2,OriginalEnvisagCompletionDt)<@PROCESSDATE 
AND ProdectDelReson<>'Court Case'
and OriginalEnvisagCompletionDt is not null
and RevisedCompletionDt is  null
and ActualCompletionDt is  null
...
----SC-11---
UPDATE B SET FlgPNPA='Y',PNPA_Reason='O_SCOD > 2 yrs, R_SCOD Blank, Actual DCCO Bank,  Court case',PNPA_DATE=NULL
FROM #TEMPTABLEPUI A
INNER JOIN PRO.AccountCal B
ON A.AccountEntityID=B.AccountEntityID
WHERE ProjectCategory in('Non Infra','CRE')
...
----SC-12---
UPDATE B SET FlgPNPA='Y',PNPA_Reason='R_SCOD > 1 yrs,  Actual DCCO Blank, No Court case',PNPA_DATE=NULL
FROM #TEMPTABLEPUI A
INNER JOIN PRO.AccountCal B
ON A.AccountEntityID=B.AccountEntityID
WHERE ProjectCategory in('Non Infra','CRE')
AND (isnull(B.FLGPNPA,'N')='N')
AND dateadd(year,1,RevisedCompletionDt)<@PROCESSDATE 
AND ProdectDelReson<>'Court Case'
and RevisedCompletionDt is not null
and ActualCompletionDt is  null

----SC-13---
UPDATE B SET FlgPNPA='Y',PNPA_Reason='R_SCOD > 2 yrs,  Actual DCCO Blank,  No Court case',PNPA_DATE=NULL
FROM #TEMPTABLEPUI A
INNER JOIN PRO.AccountCal B
ON A.AccountEntityID=B.AccountEntityID
WHERE ProjectCategory in('Non Infra','CRE')
...
----SC-14---
UPDATE B SET FlgPNPA='Y',PNPA_Reason='R_SCOD > 2 yrs,  Actual DCCO Blank,  Court case',PNPA_DATE=NULL
FROM #TEMPTABLEPUI A
INNER JOIN PRO.AccountCal B
ON A.AccountEntityID=B.AccountEntityID
WHERE ProjectCategory in('Non Infra','CRE')
