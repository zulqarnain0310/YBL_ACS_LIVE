...
DESCRIPTION : PRO.DATATOCLIENT_DEGRADE
--EXEC [PRO].[DATATOCLIENTDEGRADE]
============================================*/

CREATE PROCEDURE [pro].[DATATOCLIENTDEGRADE]
AS
BEGIN
  DECLARE @TIMEKEY INT = (SELECT TIMEKEY FROM PRO.EXTDATE_MISDB WHERE FLG = 'Y')

  DELETE  FROM  PRO.DATATOCLIENT_DEGRADE WHERE TIMEKEY=@TIMEKEY 


INSERT INTO  PRO.DATATOCLIENT_DEGRADE
...
WHERE  A.FLGDEG='Y'   and A.FinalAssetClassAlt_Key>=2
AND (ISNULL(A.Balance,0)>0 OR ISNULL(A.PrincOutStd,0)>0)
ORDER BY BranchCode DESC



--DECLARE @ColName varchar(50)='CommercialFlag_AltKey'
...
--WHERE sys.sql_modules.definition COLLATE SQL_Latin1_General_CP1_CI_AS LIKE '%'+@ColName+'%' ESCAPE '\'
--ORDER BY sys.objects.type_desc, sys.schemas.name, sys.objects.name


--select * from INFORMATION_SCHEMA.COLUMNS where COLUMN_NAME like '%CommercialFlag_AltKey%'
