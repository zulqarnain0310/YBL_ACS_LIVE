...
create PROCEDURE [pro].[InsertDataforAssetClassficationYes_ODFD]
--@TIMEKEY INT 
WITH RECOMPILE
AS
BEGIN 
...
SELECT  @StartDt=DATEADD(day,-90,DATE) FROM SysDayMAtrix
...
FROM PRO.ACCOUNTCAL  A INNER JOIN YBL_ACS_MIS..ODS_FCR_CH_OD_LIMIT B
 ...
 INTO ##AdvSecurityDetailAccountLevel FROM CURDAT.AdvSecurityDetailAccountLevel a
 INNER JOIN YBL_ACS_MIS..ODS_FCR_CH_OD_LIMIT B
 ON A.CustomerAcID=B.Cod_acct_no  and B.flg_internal_fd='Y'   
 AND A.EFFECTIVEFROMTIMEKEY<=@TIMEKEY AND A.EFFECTIVETOTIMEKEY>=@TIMEKEY
...
AND A.CustomerAcID=b.CustomerAcID
 INNER JOIN YBL_ACS_MIS..ODS_FCR_CH_OD_LIMIT c
 ON A.CustomerAcID=c.Cod_acct_no  AND c.flg_internal_fd='Y'
 where  a.Balance<=b.CurrentValue
 

 UPDATE A SET SecApp='S' FROM   PRO.ACCOUNTCAL   A  --- update SecApp='S' where SecurityValue is Zero in AccountCal but CurrentValue available in AdvSecurityDetailAccountLevel table-- changes done by Pranay 2023-04-10
 INNER JOIN  ##AdvSecurityDetailAccountLevel B 
ON  A.EFFECTIVEFROMTIMEKEY<=@TIMEKEY AND A.EFFECTIVETOTIMEKEY>=@TIMEKEY
AND A.CustomerAcID=b.CustomerAcID AND ISNULL(a.SecurityValue,0)=0 AND ISNULL(b.CurrentValue,0)>0
...
--UPDATE A SET A.ASSET_NORM ='ALWYS_STD' --- This update query is already written (reference line number 2114), commented by Pranay on 2023-04-06
--FROM PRO.ACCOUNTCAL  A INNER JOIN YBL_ACS_MIS..ODS_FCR_CH_OD_LIMIT B
-- ON A.CustomerAcID=B.Cod_acct_no  WHERE B.flg_internal_fd='Y'
...
Update A set LastCrDate=DATEADD(DAY,-90,@PROCESSINGDATE)
FROM PRO.ACCOUNTCAL A
...
select a.CustomerAcID,count(1) cnt into #Last3monthendDebit from [dbo].[AcDailyTxnDetail] A inner join
