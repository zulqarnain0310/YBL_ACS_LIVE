USE [YBL_ACS]
GO
/****** Object:  StoredProcedure [pro].[Final_AssetClass_Npadate]    Script Date: 7/3/2025 11:15:54 AM ******/
SET ANSI_NULLS ON
GO
...
=============================================*/
CREATE PROCEDURE [pro].[Final_AssetClass_Npadate_BKP_20250703]
...
IF OBJECT_ID('TEMPDB..#TempTableRefCustomerID') IS NOT NULL
	...
	/*VICE VERSA COBO RELATION ADDED BY ZAIN ON 2025-06-16*/
	UPDATE C SET C.DEGDATE=(CASE WHEN M.AssetClassification='STD' 
							
									THEN NULL ELSE M.NPADATE END)
				,C.FlgDeg=CASE WHEN M.AssetClassification='STD' 
							
									THEN NULL ELSE 'Y' END
				,C.UPGDATE=CASE WHEN M.AssetClassification='STD' 
							
									THEN @PROCESSDATE ELSE NULL END
				,C.FlgUpg=CASE WHEN M.AssetClassification='STD' 
							
									THEN 'Y' ELSE NULL END
	FROM PRO.COBORROWERDETAILS C 
			INNER JOIN DATAUPLOAD.MocCustomerDataUpload M 
								ON C.APPLICANT_FCR_CUST_ID=M.CustomerID
	...
	/**VICE VERSA COBO RELATION ADDED BY ZAIN ON 2025-06-16 END */
