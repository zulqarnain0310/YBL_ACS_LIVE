...
create  PROCEDURE [pro].[CUSTOMER_ACCOUNT_UCIFID_MASTERKEY_GENERATION_A]
AS
BEGIN
...
UPDATE YBL_ACS_MIS.[dbo].[AccountData_SmartFin]  SET SourceSystemCustomerID=FCR_CustomerID FROM YBL_ACS_MIS..AccountData_SmartFin  where  FCR_CustomerID is not null 
...
/*30012024 -AMAR FOR ECBF -  CUSTOMER WILL BE USED FROM fcr SOURCE - SO COMMENTING BELOW CODES */
/*
...
SELECT CustomerAcid FROM PRO.AccountMaster WHERE EffectiveToTimekey=49999
*/
SELECT distinct A.AccountID CustomerAcid into #TempTableCustomerAcidECBF    
--FROM YBL_ACS_MIS.[dbo].[ODS_ECBF_BORROWERMST] 
from YBL_ACS_MIS.[dbo].[AccountData_ECBF]  A
EXCEPT 
SELECT CustomerAcid FROM PRO.AccountMaster WHERE EffectiveToTimekey=49999
...
SELECT DISTINCT AccountID CustomerAcid into #TempTableCustomerAcidSFIN FROM YBL_ACS_MIS..AccountData_SmartFin 
...
SELECT DISTINCT UCIC_ID UCIFID into #TempTableUCIFIDCREDF FROM YBL_ACS_MIS..CustomerData_CA   where UCIC_ID is null
...
DELETE FROM CUSTOMERMASTER_CTE WHERE abc >1
	...
	DROP TABLE #TempTableCustomerAcidSFIN --15102023
