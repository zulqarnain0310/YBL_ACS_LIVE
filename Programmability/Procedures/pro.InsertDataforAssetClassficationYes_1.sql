...
Create PROCEDURE [pro].[InsertDataforAssetClassficationYes_1]
--@TIMEKEY INT 
WITH RECOMPILE
AS
BEGIN 
...
SELECT  SOURCESYSTEMCUSTOMERID,  MAX(CASE WHEN ASSETCLASS IN('NPA','WRITEOFF') THEN 2 WHEN ASSETCLASS ='REGULAR' THEN 1 ELSE 1 END) ASSETCLASS ,
...
SELECT A.SOURCESYSTEMCUSTOMERID,MIN(B.MAXDPD) AS MAXDPD INTO #MAXNPADPDFINNONE  FROM #TEMPTABLEFINNONE_ASSETCLASS A
...
UPDATE A SET 
			...
			,A.ACCOUNTBLKCODE2       = (CASE WHEN SOURCENAME='VISIONPLUS'  THEN    B.ACCOUNTBLKCODE2	 ELSE NULL END)
			,A.PRODUCTCODE          = B.PRODUCTCODE
FROM PRO.ACCOUNTCAL A LEFT OUTER JOIN YBL_ACS_MIS..AccountData_CA B ON A.CUSTOMERACID=B.ACCOUNTID
INNER JOIN  DIMSOURCEDB C  ON C.SOURCEALT_KEY=A.SOURCEALT_KEY
AND (C.EFFECTIVEFROMTIMEKEY<=@TIMEKEY AND C.EFFECTIVETOTIMEKEY>=@TIMEKEY)
...
UPDATE A SET A.CURRENCYALT_KEY=ISNULL(C.CURRENCYALT_KEY,62) FROM  PRO.ACCOUNTCAL 
 A LEFT OUTER  JOIN YBL_ACS_MIS..AccountData_CA B 
ON A.CUSTOMERACID=B.ACCOUNTID
INNER JOIN DIMCURRENCY C ON C.CURRENCYCODE=B.CURRENCYCODE
AND C.EFFECTIVEFROMTIMEKEY<=@TIMEKEY AND C.EFFECTIVETOTIMEKEY>=@TIMEKEY
...
FROM PRO.CUSTOMERCAL A
	 ...
	 AND B.EFFECTIVEFROMTIMEKEY<=@TIMEKEY AND B.EFFECTIVETOTIMEKEY>=@TIMEKEY
	WHERE (A.REFCUSTOMERID IS NOT NULL AND REFCUSTOMERID<>'0')  AND  ISNULL(A.SRCASSETCLASSALT_KEY,1)<>1
	 AND A.DEGREASON IS NULL
   ...
   FROM PRO.CUSTOMERCAL A
	 INNER JOIN DIMSOURCEDB  B ON B.SOURCEALT_KEY=A.SOURCEALT_KEY
	 AND B.EFFECTIVEFROMTIMEKEY<=@TIMEKEY AND B.EFFECTIVETOTIMEKEY>=@TIMEKEY
	WHERE (A.REFCUSTOMERID IS  NULL )  AND SOURCESYSTEMCUSTOMERID IS NOT NULL AND  ISNULL(A.SRCASSETCLASSALT_KEY,1)<>1
	 AND A.DEGREASON IS NULL
...
AND ValuationExpiryDate IS NOT NULL AND Advsec.UCICID IS NOT NULL
...
----WHERE  A.SourceAlt_Key=1 and A.ProductCode in('623','660','661','702','878','879','884','885','889','906','918','919')
...
UPDATE C SET Actual_DPD_OverDueSinceDt= isnull((CASE WHEN  C.Actual_OverDueSinceDt IS NOT NULL   THEN  DATEDIFF(DAY,C.Actual_OverDueSinceDt,  @PROCESSDATE) ELSE 0 END),0)+1
FROM PRO.AccountCal A
inner join DPD_Moratorium_Adjusted C ON A.AccountEntityID=C.AccountEntityID
